...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await window.storage.get("gm-lab-a1cs"); if (r?.value) setLabA1Cs(JSON.parse(r.value)); } catch {}
      setLabA1CsReady(true);
    })();
  }, []);
  useEffect(() => {
    if (!labA1CsReady) return;
    (async () => { try { await window.storage.set("gm-lab-a1cs", JSON.stringify(labA1Cs)); } catch {} })();
  }, [labA1Cs, labA1CsReady]);

  // Load medications from persistent storage
  useEffect(() => {
    (async () => {
      try { const r = await window.storage.get("gm-meds"); if (r?.value) setMeds(JSON.parse(r.value)); } catch {}
      setMedsReady(true);
    })();
  }, []);
  useEffect(() => {
    if (!medsReady) return;
    (async () => { try { await window.storage.set("gm-meds", JSON.stringify(meds)); } catch {} })();
  }, [meds, medsReady]);

  // Load saved locations from persistent storage
  useEffect(() => {
    (async () => {
      try { const r = await window.storage.get("gm-saved-locs"); if (r?.value) setSavedLocs(JSON.parse(r.value)); } catch {}
      setLocsReady(true);
    })();
  }, []);
  ...
  useEffect(() => {
    if (!locsReady) return;
    (async () => { try { await window.storage.set("gm-saved-locs", JSON.stringify(savedLocs)); } catch {} })();
  }, [savedLocs, locsReady]);
